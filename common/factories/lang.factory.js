...
app.service('langFactory', ['$http', '$q', '$rootScope', 'config', function ($http, $q, $rootScope, config) {
   ...
   /* -------------------- string formater ----------------------------- */
   // this is a copy of https://github.com/davidchambers/string-format
   // it is supposed to be used like that:
   // '"{firstName} {lastName}" <{email}>'.langFactory.format(user)
   // => '"Heiko Meier" <hmeier@googlemail.com>'
   ...
   function _create (transformers) {
      return function (template) {
         var args = Array.prototype.slice.call(arguments, 1) // because it is not a real array (array like object --> dictionary) do the 'call'
         var idx = 0
         var state = 'UNDEFINED'
